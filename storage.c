...
/* user data 4kb before zstack persistent storage */
...
settings_t settings = { 0x00000000, 0, 0};
...
void storage_init()
{
  ...
  NRF_LOG_INFO("settings mode:    %u", settings.mode);
  NRF_LOG_INFO("==============================");

  if (current_version > 0)
  {
    /* apply current settings */
    relay_set_mode(settings.mode);
  }
}
